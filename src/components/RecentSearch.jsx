...
function RecentSearch({ recentHistory, setRecentHistory, setSelectedHistory }) {
  ...
  return (
    <div>
      ...
      <div
        className={`col-span-1 dark:bg-zinc-800 bg-red-100 pt-3  h-screen 
        ${
          showHistory ? "block" : "hidden"
        } sm:block sm:w-48  fixed  top-0 left-0 z-40 `}
      >
        <h1 className="text-[16px]  dark:text-white text-zinc-800 flex text-center justify-center fixed sm:top-2 sm:left-10  top-10 sm:px-0 md:px-1 lg:px-1">
          <span>Recent Search</span>
          <button
            onClick={clearHistory}
            className="cursor-pointer hover:bg-zinc-900 bg-zinc-700 ml-2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              height="24px"
        ...
        <ul className="text-left overflow-auto mt-14 sm:mt-6">
          {recentHistory &&
